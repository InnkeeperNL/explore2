function check_achievements(item_id, skill_id, amount){
	...
	eachoa(available_achievements, function(achievement_id, achievement_info){
...
function show_achievements(){
	...
	eachoa(available_achievements, function(achievement_id, achievement_info){
		...
		parsed_achievement += 	'<div class="achievement_background" style="background-image:url(images/' + achievement_image + ')"></div>';
...
function show_spend_ap(){
	...
	var parsed_incomplete_achievements = '';
	var parsed_unstarted_achievements = '';
	eachoa(available_ap_bonusses, function(achievement_id, achievement_info){
		var current_level = 0;
		var achievement_image = '';
		if(available_items[achievement_info['background_item']] != undefined)
		{
...
function show_single_ap_bonus(){
	...
	else
	{
		...
		{
			parsed_achievement = '<button onclick="buy_ap_bonus(\'' + achievement_id + '\')">BUY</button><br/><br/>' + parsed_achievement;
		}
...
function parse_single_achievement(achievement_id, show_amount){
	...
	eachoa(achievement_info['item_bonus'], function(item_id, item_bonus){
	...
	eachoa(achievement_info['skill_bonus'], function(skill_id, skill_bonus){
	...
	parsed_achievement += 	'<div class="achievement_background" style="background-image:url(images/' + achievement_image + ')"></div>';
	parsed_achievement += 	'<div class="achievement_name">' + capitalizeFLetter(achievement_info['name']) + '</div>';
	if(show_amount == undefined || show_amount == true)
	{
		parsed_achievement += 	'<div class="achievement_progress" style="color:' + get_percent_color(current_progress, next_level_progress) + '">' + nFormatter(current_progress,3) + ' / ' + nFormatter(next_level_progress,3) + '</div>';
	}
...
function parse_achievement_message(achievement_id){
	var parsed_achievement = parse_single_achievement(achievement_id, false);
...
function get_achievement_points(){
	...
	eachoa(gamedata['achievements'], function(achievement_id, achievement_info){
...
function get_spent_points(){
	var spent_points = 0;
	eachoa(gamedata['ap_bonusses'], function(ap_id, ap_info){
...
function buy_ap_bonus(ap_id){
	...
	if(available_ap_bonusses[ap_id] != undefined && available_ap_bonusses[ap_id]['cost'] <= gamedata['ap_left'])
	{
